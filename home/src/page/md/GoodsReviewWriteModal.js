...
export default function GoodsReviewWriteModal({
  ...
  const handleFilesChange = async e => {
    ...
    try {
      ...
      for (let file of files) {
        ...
        const res = await apiClient.post('/file-system/upload', data, { headers: { 'Content-Type': 'multipart/form-data' } });
  ...
  const handleRemoveImage = async id => {
    try {
      await apiClient.patch(`/file-system/delete-image/${id}`, null, { params: { type: 'GOODSREVIEW' } });
