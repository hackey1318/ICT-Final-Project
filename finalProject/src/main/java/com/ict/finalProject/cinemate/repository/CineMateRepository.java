...
public interface CineMateRepository extends JpaRepository<CineMates, Integer> {
    ...
    //시네메이트 영화 목록
    @Query(
            value = "SELECT DISTINCT c.movie_no, m.name, m.open_date, m.post_image, m.age_grade " +
                    "FROM cine_mates c " +
    ...
    //시네메이트 영화 상세
    @Query(
            value="select c.created_at, c.max_member_count, c.meeting_date, c.movie_no, c.theater_no, c.updated_at, c.user_no, c.content, " +
