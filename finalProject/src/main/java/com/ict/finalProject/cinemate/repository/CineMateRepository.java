...
public interface CineMateRepository extends JpaRepository<CineMates, Integer> {
    ...
    @Query(value = "SELECT c.meeting_date, t.name, m.name, m.genre, m.director, m.open_date, c.no, c.max_member_count, u.nickname " +
            "FROM cine_mates AS c " +
            "LEFT JOIN movies m on c.movie_no = m.no " +
            "LEFT JOIN theaters t on c.theater_no = t.no " +
            "LEFT JOIN cine_mate_members cm on c.no = cm.cine_mate_no " +
