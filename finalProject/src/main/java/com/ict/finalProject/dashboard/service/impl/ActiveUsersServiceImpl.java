...
// 만약 로그인 활동이라면, 오늘 기록 여부 확인
    ...
    public List<ActiveUsersResponse> getDayActiveUsersList(String start, String end) {
        ...
        //날짜 범위 생성
        List<String> allDates = generateDateRange(start, end);

        //실제 로그인한 사용자가 있는 날짜
        List<ActiveUsersResponse> activeUsersData = activeUsersRepository.getDayActiveUsersList(start, end);
    ...
    //월별 활동인원수
    @Override
    public List<ActiveUsersResponse> getMonthActiveUsersList(String firstDayOfLastYearMonth, String lastDayOfPrevMonth) {

        //해당 월에 활동한 사용자 데이터 가져오기
        return activeUsersRepository.getMonthActiveUsersList(firstDayOfLastYearMonth, lastDayOfPrevMonth);
    }
